fastapi-auth-integration-system-prompt-v2.txt
================================================================================
...
def require_permission(
    ...
    *,
    ...
    checks: list[Callable[[AuthenticatedUser, Request], bool]] | None = None,
    ...
    Parameters:
        ...
        check: Single callback function (user, request) -> bool, runs AFTER
               permission verification but BEFORE route handler.
        checks: Multiple callback functions. Combined with check_mode.
        check_mode: "all" = all checks must pass (AND), "any" = any check
                    passes (OR). Default: "all".

    Raises: PermissionDeniedError if permission not granted or check fails

    Example with single check:
        require_permission(auth, "resource.read", check=belongs_to_org)

    Example with multiple checks:
        require_permission(auth, "resource.write",
            checks=[is_resource_owner, is_not_suspended],
            check_mode="all")
    """
    ...
    *permissions: str,
    ...
    Parameters:
        *permissions: Variable permission strings
        check: Single callback (user, request) -> bool
        checks: Multiple callbacks, combined with check_mode
        check_mode: "all" or "any" (default: "all")

    Example: require_any_permission(auth, "admin:*", "users:read")

    Example with checks:
        require_any_permission(auth,
...
5.2.1 Check Callbacks (Added Feature)
--------------------------------------

Check callbacks are functions that run AFTER permission verification but BEFORE
the route handler. They enable contextual authorization beyond static permissions.

Signature: (user: AuthenticatedUser, request: Request) -> bool
Return True to allow, False to deny (raises PermissionDeniedError).

Common patterns:

```python
# Org membership check — user must be in the org specified by the request
...
class AuthGuard:
    ...
    #   "client" — checks permissions from JWT claims (fast, offline, but stale
    #              until token refresh; ~15 min lag on permission changes)
    #   "server" — calls auth service /permissions/check endpoint per request
    #              (real-time revocation, recommended for production)
...
AB0T_AUTH_PERMISSION_CHECK_MODE=client  # "client" (JWT claims) or "server" (API call)
...
auth = AuthGuard(
    ...
    org_id="my-org",
    debug=True,
    permission_check_mode="server",  # "client" or "server"
)
...
# ============================================================================
# Check Callbacks (run after permission check, before route handler)
# ============================================================================

def belongs_to_org(user: AuthenticatedUser, request: Request) -> bool:
    """User must be in the org specified by the request."""
    org_id = request.path_params.get("org_id") or request.query_params.get("org_id")
    if not org_id:
        return True
...
OrgReader = Annotated[AuthenticatedUser, Depends(
    require_permission(auth, "data:read", check=belongs_to_org)
)]
...
================================================================================
CHANGELOG — UPDATES ADDED 2026-02-13
================================================================================

The following features were added to this document to match the current library
implementation (verified against Resource Service production code):

1. CHECK CALLBACKS on dependency functions (Section 5.2, 5.2.1):
   - `check` parameter: single callback (user, request) -> bool
   - `checks` parameter: list of callbacks
   - `check_mode` parameter: "all" (AND) or "any" (OR)
   - Added to: require_permission, require_any_permission
   - Full callback pattern documentation with examples

2. PERMISSION_CHECK_MODE on AuthGuard (Section 6.2):
   - New `permission_check_mode` parameter: "client" or "server"
   - "client" = check JWT claims locally (fast, but stale until token refresh)
   - "server" = call auth service API per request (real-time revocation)
   - Also added AB0T_AUTH_PERMISSION_CHECK_MODE env var (Section 8.1)

3. Updated complete application example (Section 10):
   - Added check callback example with belongs_to_org
   - Added OrgReader type alias showing check= usage

These additions bring this document in sync with:
- ab0t_auth_integration_skill_20260213.md (comprehensive integration skill)
- Resource Service production code (app/auth.py)
