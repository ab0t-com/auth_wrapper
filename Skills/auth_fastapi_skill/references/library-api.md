...
class AuthGuard:
    ...
    # Opt-in: map an auth revocation event -> the invalidate_* methods above,
    # so a subscribed consumer invalidates immediately instead of waiting for
    # cache TTL. Additive/no-op unless called. Returns RevocationResult.
